...
static inline struct tcphdr* tcp6_hdr(mbuf_t skb)
{
    ...
    while (nexthdr != IPPROTO_TCP) {
        if (cursor + 2 > limit)
            return NULL;
        nexthdr = cursor[0];
        cursor += (size_t)(cursor[1] + 1) * 8;
        if (cursor > limit)
            return NULL;
    }
    return (struct tcphdr*)cursor;
}
                            ...
                            struct sk_buff *skb)
{
    ...
    if (unlikely(mbuf_copyback(skb, mbuf_pkthdr_len(skb), size,
                      va, MBUF_DONTWAIT))) {
        // copyback failed: mbuf is in an inconsistent state.
        // Free it and signal the caller to drop this descriptor.
                                           ...
                                           struct sk_buff *skb)
{
	...
	if (likely(igb_add_rx_frag(rx_ring, rx_buffer, rx_desc, skb))) {
		...
		/* igb_add_rx_frag already freed skb on mbuf_copyback failure */
		rx_buffer->page = NULL;
		return NULL;
	}
